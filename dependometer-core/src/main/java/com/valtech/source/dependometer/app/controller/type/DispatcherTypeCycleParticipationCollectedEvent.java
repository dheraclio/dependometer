...
final class DispatcherTypeCycleParticipationCollectedEvent extends Dispatcher
{
   protected void dispatch(Object handler, EventIf event)
   {
      assert handler != null;
      assert event != null;
      ((HandleTypeCycleParticipationCollectedEventIf)handler)
         .handleEvent((com.valtech.source.dependometer.app.controller.type.TypeCycleParticipationCollectedEvent)event);
   }
}
