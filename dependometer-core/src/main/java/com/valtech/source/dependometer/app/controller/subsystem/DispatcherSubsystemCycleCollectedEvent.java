...
final class DispatcherSubsystemCycleCollectedEvent extends Dispatcher
{
   protected void dispatch(Object handler, EventIf event)
   {
      assert handler != null;
      assert event != null;
      ((HandleSubsystemCycleCollectedEventIf)handler)
         .handleEvent((com.valtech.source.dependometer.app.controller.subsystem.SubsystemCycleCollectedEvent)event);
   }
}
