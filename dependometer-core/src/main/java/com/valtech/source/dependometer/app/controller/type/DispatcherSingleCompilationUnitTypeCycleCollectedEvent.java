...
final class DispatcherSingleCompilationUnitTypeCycleCollectedEvent extends Dispatcher
{
   protected void dispatch(Object handler, EventIf event)
   {
      assert handler != null;
      assert event != null;
      ((HandleSingleCompilationUnitTypeCycleCollectedEventIf)handler)
         .handleEvent((com.valtech.source.dependometer.app.controller.type.SingleCompilationUnitTypeCycleCollectedEvent)event);
   }
}
