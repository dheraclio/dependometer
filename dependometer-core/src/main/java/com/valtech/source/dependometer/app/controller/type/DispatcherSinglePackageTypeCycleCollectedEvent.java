...
final class DispatcherSinglePackageTypeCycleCollectedEvent extends Dispatcher
{
   protected void dispatch(Object handler, EventIf event)
   {
      assert handler != null;
      assert event != null;
      ((HandleSinglePackageTypeCycleCollectedEventIf)handler)
         .handleEvent((com.valtech.source.dependometer.app.controller.type.SinglePackageTypeCycleCollectedEvent)event);
   }
}
