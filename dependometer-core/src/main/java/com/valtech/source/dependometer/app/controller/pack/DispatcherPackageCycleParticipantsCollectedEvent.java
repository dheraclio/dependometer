...
final class DispatcherPackageCycleParticipantsCollectedEvent extends Dispatcher
{
   protected void dispatch(Object handler, EventIf event)
   {
      assert handler != null;
      assert event != null;
      ((HandlePackageCycleParticipantsCollectedEventIf)handler)
         .handleEvent((com.valtech.source.dependometer.app.controller.pack.PackageCycleParticipantsCollectedEvent)event);
   }
}
