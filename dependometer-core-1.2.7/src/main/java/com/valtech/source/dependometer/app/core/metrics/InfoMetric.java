 ...
 */
final class InfoMetric extends Metric
{
   private final String m_Info;

   protected InfoMetric(MetricEnum shortName, String info)
   {
      super(shortName);
      assert info != null;
   ...
   {
      assert metric != null;
      assert metric instanceof InfoMetric;
      return m_Info.compareTo(((InfoMetric)metric).m_Info);
   }
}