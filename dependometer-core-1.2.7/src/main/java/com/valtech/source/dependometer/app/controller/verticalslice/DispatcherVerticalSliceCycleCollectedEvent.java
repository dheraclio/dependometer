...
final class DispatcherVerticalSliceCycleCollectedEvent extends Dispatcher
{
   protected void dispatch(Object handler, EventIf event)
   {
      assert handler != null;
      assert event != null;
      ((HandleVerticalSliceCycleCollectedEventIf)handler)
         .handleEvent((com.valtech.source.dependometer.app.controller.verticalslice.VerticalSliceCycleCollectedEvent)event);
   }
}
